...
/**
 * Get all post/page meta or just by meta key.
 */
function conditional_blocks_get_post_meta( $key = false ) {

	$id = get_the_ID();

	if ( empty( $id ) || empty( $meta = get_post_meta( $id ) ) ) {
		return array();
	}
 ...
 */
function conblocks_get_user_roles() {
 ...
 */
function conblock_parse_user_agent( $u_agent = null ) {
	if ( $u_agent === null ) {
