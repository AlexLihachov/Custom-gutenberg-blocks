...
if ( ! class_exists( 'Rri_Register_Blocks' ) ) {
	...
	class Rri_Register_Blocks {
		...
		public static function init_hook() {
			...
			/* Script to extend banner block */
			wp_register_script( 'rri-banner-block-js', RRI_URL . 'js/rri-banner-block.js', array( 'wp-i18n', 'wp-element', 'wp-blocks', 'wp-components', 'wp-editor' ), filemtime( RRI_DIR . 'js/rri-banner-block.js' ), false );

			wp_localize_script( 'rri-banner-block-js', 'rri_blocks', array( 'image_url' => RRI_URL . 'images/' ) );

			register_block_type(
				RRI_TEXTDOMAIN . '/rri-banner-block',
				array(
					'editor_script'   => 'rri-banner-block-js',
					'render_callback' => 'Rri_Banner_Block::render_banner_block',
					'attributes'      =>  $attributes
				)
			);


            /* Script to extend header block */
            wp_register_script( 'rri-header-block-js', RRI_URL . 'js/rri-header-block.js', array( 'wp-i18n', 'wp-element', 'wp-blocks', 'wp-components', 'wp-editor' ), filemtime( RRI_DIR . 'js/rri-header-block.js' ), false );

            wp_localize_script( 'rri-header-block-js', 'rri_blocks', array( 'image_url' => RRI_URL . 'images/' ) );

            register_block_type(
                RRI_TEXTDOMAIN . '/rri-header-block',
                array(
                    'editor_script'   => 'rri-header-block-js',
                    'render_callback' => 'Rri_Header_Block::render_header_block',
                    'attributes'      =>  $attributes
                )
            );
		}
