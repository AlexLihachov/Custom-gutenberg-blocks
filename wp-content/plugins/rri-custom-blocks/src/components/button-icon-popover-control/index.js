    ...
    render() {
        return (
            <BaseControl
                ...
                label={!this.props.onToggle &&
                <label htmlFor={`rri-button-icon-control__edit-${this.instanceId}`}>{this.props.label}</label>}
                id="rri-button-icon-control"
                className={classnames('rri-button-icon-control', this.props.className)}
            >
                {this.props.onToggle && (
                    <ToggleControl
                ...
                <div className="rri-button-icon-control__wrapper">
                    {this.props.allowReset && (
                        <IconButton
                            onClick={this.props.onReset}
                            className="rri-button-icon-control__reset"
                            label={__('Reset', i18n)}
                            icon="image-rotate"
                        />
                    )}
                    <IconButton
                        ...
                        className="rri-button-icon-control__edit"
                        label={__('Edit', i18n)}
                        isDefault
                        icon="edit"
                        id={`rri-button-icon-control__edit-${this.instanceId}`}
                        ref={this.buttonRef}
                    />
                    {this.state.open && (
                        <Popover
                            className="rri-button-icon-control__popover"
