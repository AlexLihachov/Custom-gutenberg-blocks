...
class PanelTabs extends Component {
	constructor() {
		...
		if(this.props.blockProps.name !== "rri/buttons"){
			this.state = {
	...
	updateSidebarPanelTab(tab) {
		...
		sidebarPanel.setAttribute('data-rri-tab', tab);
		sidebarPanel.closest('.edit-post-sidebar').classList.add('rri--has-panel-tabs')
	}
	...
	componentWillUnmount() {
		const sidebarPanel = this.containerDiv.current.closest('.components-panel');
		sidebarPanel.removeAttribute('data-rri-tab');
		sidebarPanel.closest('.edit-post-sidebar').classList.remove('rri--has-panel-tabs');
	...
	render() {
		...
		return (
			...
			>
				<div className="rri-panel-tabs__wrapper">
					{TABS.map(({
